...
class SGL(GeneralRecommender):
    ...
    def rand_sample(self, high, size=None, replace=True):
        # high: upper limit of index value
        # size: array size after sampling
        a = np.arange(high)
    ...
    def create_adjust_matrix(self, is_sub: bool):
        ...
        if not is_sub:
            ...
            if self.type == 'ND':
                ...
                keep_item = self.rand_sample(len(self._user), size=int(len(self._user) * (1 - self.drop_ratio)), replace=False)
    ...
    def csr2tensor(self, matrix: sp.csr_matrix):
        matrix = matrix.tocoo()
        x = torch.sparse.FloatTensor(torch.LongTensor(np.array([matrix.row, matrix.col])),
                                     torch.FloatTensor(matrix.data.astype(np.float32)), matrix.shape).tolist()(self.device)
        return x
